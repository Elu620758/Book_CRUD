...
class App extends Component {
  ...
  componentWillMount() {  // React Lifecycle
    this._refreshBooks(); // Call back Method
  }
  toggleNewBookModal() {
    this.setState({
  ...
  addBook() {
    axios.post('http://localhost:3004/books', this.state.newBookData).then((response) => {
  ...
  updateBook() {
    ...
    axios.put('http://localhost:3004/books/' + this.state.editBookData.id, {
      title, rating
    }).then((response) => {
      this._refreshBooks();
  ...
  _refreshBooks() {
    axios.get('http://localhost:3004/books').then((response) => {
      this.setState({
        books: response.data
      })
    });
